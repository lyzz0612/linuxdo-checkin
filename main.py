...
class LinuxDoBrowser:
    ...
    def browse_post(self, page):
        ...
        for _ in range(40):
            ...
            elif prev_url == current_url and at_bottom:
                if not reach_bottom_once:
                    # 额外再等3秒，如果还是底部，则退出浏览
                    logger.info("首次到达页面底部，额外等待3秒...")
                    reach_bottom_once = True
                    time.sleep(3)
                else:
                    logger.success("已到达页面底部，退出浏览")
                    break
            # 动态随机等待
            wait_time = random.uniform(1, 2)  # 随机等待 1-2 秒
